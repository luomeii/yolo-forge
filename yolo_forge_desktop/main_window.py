...
from .panels import (
    ...
    SettingsPanel, TaskPanel, TrainerPanel,
)
...
class MainWindow(QMainWindow):
    ...
    def _init_agent(self) -> None:
        ...
        try:
            ...
            # v0.10.10: 加载会话列表和历史消息
            self.agent_panel._current_conv_id = conv_id
            self.agent_panel._load_conversation_messages(conv_id)
            self.agent_panel._refresh_conversation_list()
    ...
    def _on_agent_submit(self, agent_name: str, text: str) -> None:
        ...
        # v0.10.8: 信号连接 — 方法名和信号名对齐
        from .panels.agent_worker import _AgentWorker
        worker = _AgentWorker(text, self._unified_agent)

        worker.tool_start_sig.connect(self._on_tool_start_sig)
        worker.tool_end_sig.connect(self._on_tool_end_sig)
        worker.train_log_sig.connect(self._on_train_log)
        worker.train_complete_sig.connect(self._on_train_complete)
        worker.permission_request_sig.connect(self._on_permission_request)
        worker.finished_reply_sig.connect(lambda reply, conv: self._on_agent_reply(reply))
        worker.failed_sig.connect(lambda err: self._on_agent_error(err))
    ...
    def _switch_conversation(self, conv_id: str) -> None:
        ...
        try:
            ...
            print(f"[WARN] 切换会话失败: {e}")

    def _on_agent_step(self, step: str) -> None:
        self.status_label.setText(step)

    def _on_tool_start_sig(self, name: str, args_str: str) -> None:
        """v0.10.8: worker 信号收到, 显示工具调用."""
        self.agent_panel.append_message("tool", f"▶ {name}({args_str})")

    def _on_tool_end_sig(self, name: str, preview: str) -> None:
        """v0.10.8: worker 信号收到, 显示工具结果."""
        self.agent_panel.append_message("tool", f"✓ {name}: {preview}")
    ...
    def _on_agent_reply(self, reply: str) -> None:
        ...
        self.agent_panel.append_message("assistant", reply, ts)
    ...
    def _on_permission_request(self, tool_name: str, args_json: str) -> None:
        """v0.10.9: 权限审批."""
        ...
        if tool_name == "run_shell":
            ...
            if self._agent_worker:
                self._agent_worker.set_permission_result(True)
            return

        self.agent_panel.append_message("system", f"[权限请求] {msg}")
        ...
        if approved:
            ...
            _tools._permission_callback = lambda name, a: True

        if self._agent_worker:
            self._agent_worker.set_permission_result(approved)
