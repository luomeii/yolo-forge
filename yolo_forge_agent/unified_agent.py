...
from .conversation_store import Message, get_store
...
class UnifiedAgent:
    ...
    def __init__(
        ...
        if conv_id is None:
            meta = self.store.create_conversation()
            self.conv_id = meta.id
        elif self.store.get_meta(conv_id) is None:
            meta = self.store.create_conversation()
            self.conv_id = meta.id
        else:
    ...
    def chat(self, user_text: str) -> str:
        ...
        # 存用户消息到会话存储
        self.store.append_message(self.conv_id, "user", user_text)

        # 构造给 LLM 的 messages 列表
        ...
        try:
            ...
            self.store.append_message(self.conv_id, "assistant", reply)
            return reply
    ...
    def _build_llm_messages(self) -> List[ChatMessage]:
        ...
        messages = [ChatMessage(role="system", content=system_with_time)]
        history = self.store.load_messages(self.conv_id)
        for m in history:
            if m.role in ("user", "assistant") and m.timestamp:
                content = f"[{m.timestamp}] {m.content}"
            else:
                content = m.content
    ...
    def get_messages(self):
        return self.store.load_messages(self.conv_id)
